...
namespace PowerStore.Services.Installation
{
    public partial class UpgradeService : IUpgradeService
    {
        ...
        private async Task From400To410()
        {
            ...
            var _keepAliveScheduleTask = keepliveTask.Table.Where(x => x.Type == "PowerStore.Services.Tasks.KeepAliveScheduleTask").FirstOrDefault();
        ...
        private async Task From430To440()
        {
            ...
            var messagetemplateService = _serviceProvider.GetRequiredService<PowerStore.Services.Messages.IMessageTemplateService>();
        ...
        private async Task From440To450()
        {
            ...
            foreach (var task in tasks.Table)
            {
                ...
                if (task.Type == "PowerStore.Services.Tasks.ClearLogScheduleTask")
                {
                    task.Type = "PowerStore.Services.Tasks.ClearLogScheduleTask, PowerStore.Services";
                    await tasks.UpdateAsync(task);
                }
            }
            #endregion
