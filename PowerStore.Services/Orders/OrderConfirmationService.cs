...
using PowerStore.Services.Events.Extensions;
    ...
    public class OrderConfirmationService : IOrderConfirmationService
    {
        ...
        protected virtual async Task<ProcessPaymentResult> PrepareProcessPaymentResult(ProcessPaymentRequest processPaymentRequest, PlaceOrderContainter details)
        {
            ...
            if (!skipPaymentWorkflow)
            {
                ...
                if (!paymentMethod.IsPaymentMethodActive(_paymentSettings))
                    throw new PowerStoreException("Payment method is not active");
                    ...
                    if (details.IsRecurringShoppingCart)
                    {
                        ...
                        switch (recurringPaymentType)
                        {
                            case RecurringPaymentType.NotSupported:
                                throw new PowerStoreException("Recurring payments are not supported by selected payment method");
                            case RecurringPaymentType.Manual:
                            case RecurringPaymentType.Automatic:
                                processPaymentResult = await _paymentService.ProcessRecurringPayment(processPaymentRequest);
                                break;
                            default:
                                throw new PowerStoreException("Not supported recurring payment type");
                        }
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        throw new PowerStoreException("No recurring products");
                    }
                }
            }
            else
            ...
            if (processPaymentResult == null)
                throw new PowerStoreException("processPaymentResult is not available");
        ...
        protected virtual async Task<PlaceOrderContainter> PreparePlaceOrderDetails(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            if (details.Customer.IsGuest() && !_orderSettings.AnonymousCheckoutAllowed)
                throw new PowerStoreException("Anonymous checkout is not allowed");

            //billing address
            if (details.Customer.BillingAddress == null)
                throw new PowerStoreException("Billing address is not provided");

            if (!CommonHelper.IsValidEmail(details.Customer.BillingAddress.Email))
                throw new PowerStoreException("Email is not valid");
            ...
            if (!string.IsNullOrEmpty(details.BillingAddress.CountryId))
            {
                ...
                if (country != null)
                    if (!country.AllowsBilling)
                        throw new PowerStoreException(string.Format("Country '{0}' is not allowed for billing", country.Name));
            }
            ...
            if (!details.Cart.Any())
                throw new PowerStoreException("Cart is empty");
            ...
            if (warnings.Any())
            {
                ...
                throw new PowerStoreException(warningsSb.ToString());
            }
            ...
            if (!minOrderSubtotalAmountOk)
            {
                ...
                throw new PowerStoreException(string.Format(_localizationService.GetResource("Checkout.MinOrderSubtotalAmount"), _priceFormatter.FormatPrice(minOrderSubtotalAmount, true, false)));
            }
            ...
            {
                throw new PowerStoreException(_localizationService.GetResource("Checkout.MinMaxOrderTotalAmount"));
            }
            ...
            if (shoppingCartRequiresShipping)
            {
                ...
                else
                {
                    if (details.Customer.ShippingAddress == null)
                        throw new PowerStoreException("Shipping address is not provided");
                    ...
                    if (!String.IsNullOrEmpty(details.ShippingAddress.CountryId))
                    {
                        ...
                        if (country != null)
                            if (!country.AllowsShipping)
                                throw new PowerStoreException(string.Format("Country '{0}' is not allowed for shipping", country.Name));
                    }
                }
            ...
            if (!orderShippingTotalInclTax.HasValue || !orderShippingTotalExclTax.HasValue)
                throw new PowerStoreException("Shipping total couldn't be calculated");
            ...
            if (!orderTotal.HasValue)
                throw new PowerStoreException("Order total couldn't be calculated");
            ...
            if (details.IsRecurringShoppingCart)
            {
                ...
                if (!string.IsNullOrEmpty(recurringCyclesError))
                    throw new PowerStoreException(recurringCyclesError);
