        ...
        public virtual async Task ProcessNextRecurringPayment(RecurringPayment recurringPayment)
        {
            ...
            try
            {
                if (!recurringPayment.IsActive)
                    throw new PowerStoreException("Recurring payment is not active");

                var initialOrder = recurringPayment.InitialOrder;
                if (initialOrder == null)
                    throw new PowerStoreException("Initial order could not be loaded");

                var customer = await _customerService.GetCustomerById(initialOrder.CustomerId);
                if (customer == null)
                    throw new PowerStoreException("Customer could not be loaded");

                var nextPaymentDate = recurringPayment.NextPaymentDate;
                if (!nextPaymentDate.HasValue)
                    throw new PowerStoreException("Next payment date could not be calculated");
                ...
                if (result.Success)
                {
                    if (result.PlacedOrder == null)
                        throw new PowerStoreException("Placed order could not be loaded");
                ...
                else
                {
                    ...
                    throw new PowerStoreException(error);
                }
            }
            catch (Exception exc)
            {
