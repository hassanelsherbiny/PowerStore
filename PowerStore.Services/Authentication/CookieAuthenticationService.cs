...
namespace PowerStore.Services.Authentication
{
    ...
    /// </summary>
    public partial class CookieAuthenticationService : IPowerStoreAuthenticationService
    {
        #region Const

        private string CUSTOMER_COOKIE_NAME => $"{_PowerStoreConfig.CookiePrefix}Customer";
        ...
        public virtual async Task SignIn(Customer customer, bool isPersistent)
        {
            ...
            if (!string.IsNullOrEmpty(customer.Username))
                claims.Add(new Claim(ClaimTypes.Name, customer.Username, ClaimValueTypes.String, _PowerStoreConfig.CookieClaimsIssuer));

            if (!string.IsNullOrEmpty(customer.Email))
                claims.Add(new Claim(ClaimTypes.Email, customer.Email, ClaimValueTypes.Email, _PowerStoreConfig.CookieClaimsIssuer));
            ...
            if (string.IsNullOrEmpty(passwordtoken))
            {
                ...
                claims.Add(new Claim(ClaimTypes.UserData, passwordguid, ClaimValueTypes.String, _PowerStoreConfig.CookieClaimsIssuer));
            }
            else
                claims.Add(new Claim(ClaimTypes.UserData, passwordtoken, ClaimValueTypes.String, _PowerStoreConfig.CookieClaimsIssuer));

            //create principal for the current authentication scheme
            var userIdentity = new ClaimsIdentity(claims, PowerStoreCookieAuthenticationDefaults.AuthenticationScheme);
            ...
            //sign in
            await _httpContextAccessor.HttpContext.SignInAsync(PowerStoreCookieAuthenticationDefaults.AuthenticationScheme, userPrincipal, authenticationProperties);
        ...
        public virtual async Task SignOut()
        {
            ...
            await _httpContextAccessor.HttpContext.SignOutAsync(PowerStoreCookieAuthenticationDefaults.AuthenticationScheme);
        }
        ...
        public virtual async Task<Customer> GetAuthenticatedCustomer()
        {
            ...
            var authenticateResult = await _httpContextAccessor.HttpContext.AuthenticateAsync(PowerStoreCookieAuthenticationDefaults.AuthenticationScheme);
            ...
            if (_customerSettings.UsernamesEnabled)
            {
                ...
                var usernameClaim = authenticateResult.Principal.FindFirst(claim => claim.Type == ClaimTypes.Name
                    && claim.Issuer.Equals(_PowerStoreConfig.CookieClaimsIssuer, StringComparison.InvariantCultureIgnoreCase));
