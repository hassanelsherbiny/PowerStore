...
function displayBarNotification(message, messagetype, timeout) {
    if (messagetype == 'error') {
        toastHTML = '<b-toast id="PowerStoreToast" auto-hide-delay=' + timeout +' variant="danger" title=' + messagetype +'>'+ message +'</b-toast>'
    } else {
        toastHTML = '<b-toast id="PowerStoreToast" auto-hide-delay=' + timeout +' variant="info" title=' + messagetype+'>' + message + '</b-toast>'
    }
    ...
    new Vue({
        ...
        methods: {
            toast: function () {
                this.$bvToast.show('PowerStoreToast')
            }
        },
        mounted: function () {
            this.toast();
...
function displayPopupNotification(message, messagetype) {
    ...
    if ((typeof message) == 'string') {
        htmlcode = '<b-modal ref="PowerStoreModal" id="PowerStoreModal" centered hide-footer hide-header><b-alert class="mb-0" show>' + message + '</b-alert></b-modal>';
        ...
        new Vue({
            ...
            methods: {
                showModal: function () {
                    this.$refs['PowerStoreModal'].show()
                },
            },
            mounted: function () {
                var self = this;
