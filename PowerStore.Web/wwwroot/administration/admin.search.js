...
Admin.Search = (function () {
    var itemTemplate = function (data) {
        var path = function () {
            if (data.PowerStoreParent) {
                return $("<p/>").text([data.PowerStoreParent, data.parent].join(" > ")).html();
            }
            else {
                return data.parent;
            }
                ...
                case 1:
                    {
                        ...
                        map[href] = { title: title, link: href, parent: null, PowerStoreParent: null };
                        break;
                    }
                // items in level two, these items have parent but have not PowerStore parent
                case 3:
                    {
                        href = $(parents).eq(0).find("a").attr("href");
                        title = $(parents).eq(0).find("a").find("span").html();
                        parent = $(parents).eq(2).find("a").find("span").html();
                        map[href] = { title: title, link: href, parent: parent, PowerStoreParent: null };
                        break;
                    }
                // items in level three, these items have both parent and PowerStore parent
                case 5:
                    {
                        href = $(parents).eq(0).find("a").attr("href");
                        title = $(parents).eq(0).find("a").find("span").html();
                        parent = $(parents).eq(2).find("a").find("span").html();
                        PowerStoreParent = $(parents).eq(4).find("a").find("span").html();
                        map[href] = { title: title, link: href, parent: parent, PowerStoreParent: PowerStoreParent };
                        break;
                    }
                default: break;
            }
